...
gdat = s1(29:9770);
repet = ceil(sim_time/length(gdat));
snew = repmat(gdat,[1,repet]);
snew(sim_time+1:end) = [];
incr = linspace(0,perc*mean(s1),length(snew));
s1 = snew+incr;
...
snew = repmat(gdat,[1,repet]);
snew(sim_time+1:end) = [];
incr = linspace(0,perc*mean(s2),length(snew));
s2 = snew+incr;
...
snew = repmat(gdat,[1,repet]);
snew(sim_time+1:end) = [];
incr = linspace(0,perc*mean(s3),length(snew));
s3 = snew+incr;
...
snew = repmat(gdat,[1,repet]);
snew(sim_time+1:end) = [];
incr = linspace(0,perc*mean(s4),length(snew));
s4 = snew+incr;
